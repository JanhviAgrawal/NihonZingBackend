...
const { sendOTPMail , sendSignupOTPMail, sendWelcomeMail } = require("../../../utils/mailer");
const { MSG } = require("../../../utils/msg");
const { errorResponse, successResponse } = require("../../../utils/response");
const { broadcastLeaderboard } = require("../../../utils/socket");
...
module.exports.registerUser = async (req, res) => {
    try {
        ...
        const existing = await userAuthService.fetchSingleUser(
        ...
        if (existing) {
            if (existing.isActive) {
                // Fully registered & verified — block as before
            ...
            // Inactive/unverified user already exists.
            // Resend a fresh signup OTP.
            const OTP = String(
            ...
            const expireOTPTime = new Date(
                Date.now() + 1000 * 60 * 5
            ); // 5 minutes

            try {
                // Signup OTP email
                await sendSignupOTPMail(req.body.email, OTP);

                await userAuthService.updateUser(existing._id, {
                    OTP: hashedOTP,
        ...
        try {
            ...
            await userAuthService.updateUser(newUser._id, {
                OTP: hashedOTP,
                OTP_Expire: expireOTPTime,
                attempt: 0,
                attempt_expire: null
            ...
            await userAuthService.deleteUser(newUser._id);
...
module.exports.verifyOTP = async (req, res) => {
    try {
        ...
        await userAuthService.updateUser(user._id, {
            ...
            verify_attempt_expire: new Date(
                Date.now() + 1000 * 60 * 60
            )
        });
        ...
        // Capture active status BEFORE marking the user as active
        ...
        // Do not block OTP verification if email sending fails
        if (!wasAlreadyActive) {
            sendWelcomeMail(
                user.email,
...
module.exports.addXP = async (req, res) => {
    try {
        ...
        if (amount <= 0) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, "Invalid XP amount."));
        }

        const updatedUser = await userAuthService.incrementXP(req.user._id, amount);

        // Push updated leaderboard to all connected clients
        broadcastLeaderboard();

        return res.status(statusCode.OK).json(successResponse(statusCode.OK, false, "XP updated successfully", updatedUser));
    } catch (err) {
        console.log("Error : ", err);
        return res.status(statusCode.INTERNAL_SERVER_ERROR).json(errorResponse(statusCode.INTERNAL_SERVER_ERROR, true, err.message));
    }
